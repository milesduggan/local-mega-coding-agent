...
class ChatPanel {
    ...
    normalizedTask = null;
    constructor(panel, extensionUri, backend) {
        this.panel = panel;
    ...
    static createOrShow(extensionUri, backend) {
        const column = vscode.ViewColumn.Beside;
        if (ChatPanel.currentPanel) {
            ChatPanel.currentPanel.panel.reveal(column);
            return ChatPanel.currentPanel;
        }
        const panel = vscode.window.createWebviewPanel("aiAgentChat", "AI Agent Chat", column, {
            enableScripts: true,
    ...
    async handleUserMessage(text) {
        ...
        try {
            ...
            const messageWithContext = fileSummaries
                ? `${text}\n\n${fileSummaries}`
                : text;
            const response = await this.backend.chat(messageWithContext, this.conversationHistory);
    ...
    async handleQuickExecute(task) {
        if (!task.trim()) {
            vscode.window.showWarningMessage("Please enter a task description.");
            return;
        }
        if (this.selectedFiles.size === 0) {
            vscode.window.showWarningMessage("No files selected. Right-click files in the explorer to add them.");
            return;
        }
        this.panel.webview.postMessage({ type: "setLoading", loading: true });
        // Show the task in UI
        ...
        try {
            ...
            // Optional review
            const reviewChoice = await vscode.window.showInformationMessage("Diff generated. Run critic review?", "Yes", "No");
            if (reviewChoice === "Yes") {
    ...
    async handleProceed() {
        ...
        try {
            ...
            const condensedTask = await this.backend.normalizeTask(this.conversationHistory);
            this.panel.webview.postMessage({
                type: "addMessage",
                role: "assistant",
